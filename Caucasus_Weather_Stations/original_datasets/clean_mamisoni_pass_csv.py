...
MAMISONI_CSV = BASE_DIR / "Mamisoni Pass 2854m Lat42d42_ Long43d47_.csv"


def load_mamisoni_csv(path: Path, meta: StationMetadata) -> pd.DataFrame:
    df = pd.read_csv(
        path,
...
def aggregate_mamisoni_monthly(df_daily: pd.DataFrame) -> pd.DataFrame:
    df = df_daily.copy()
...
def main() -> None:
    if not MAMISONI_CSV.exists():
        raise FileNotFoundError(f"Mamisoni Pass CSV not found at {MAMISONI_CSV}")

    meta = parse_station_metadata(MAMISONI_CSV)
    raw = load_mamisoni_csv(MAMISONI_CSV, meta)
    ...
    monthly = aggregate_mamisoni_monthly(daily)

    DERIVED_DIR.mkdir(parents=True, exist_ok=True)
    daily.to_csv(DERIVED_DIR / "mamisoni_pass_daily_clean.csv", index=False)
    monthly.to_csv(DERIVED_DIR / "mamisoni_pass_monthly_clean.csv", index=False)
