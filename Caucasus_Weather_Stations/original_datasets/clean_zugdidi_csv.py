...
ZUGDIDI_CSV = BASE_DIR / "Zugdidi 118m Lat42d31_ Long41d53_.csv"


def load_zugdidi_csv(path: Path) -> pd.DataFrame:
...
def aggregate_zugdidi_monthly(df_daily: pd.DataFrame) -> pd.DataFrame:
    df = df_daily.copy()
...
def main() -> None:
    if not ZUGDIDI_CSV.exists():
        raise FileNotFoundError(f"Zugdidi CSV not found at {ZUGDIDI_CSV}")

    raw = load_zugdidi_csv(ZUGDIDI_CSV)
    ...
    monthly = aggregate_zugdidi_monthly(daily)

    DERIVED_DIR.mkdir(parents=True, exist_ok=True)
    daily.to_csv(DERIVED_DIR / "zugdidi_daily_clean.csv", index=False)
    monthly.to_csv(DERIVED_DIR / "zugdidi_monthly_clean.csv", index=False)
