...
TSAGERI_CSV = BASE_DIR / "Tsageri 500m Lat42d38_ Long42d46_.csv"


def load_tsageri_csv(path: Path) -> pd.DataFrame:
...
def aggregate_tsageri_monthly(df_daily: pd.DataFrame) -> pd.DataFrame:
    df = df_daily.copy()
...
def main() -> None:
    if not TSAGERI_CSV.exists():
        raise FileNotFoundError(f"Tsageri CSV not found at {TSAGERI_CSV}")

    raw = load_tsageri_csv(TSAGERI_CSV)
    ...
    monthly = aggregate_tsageri_monthly(daily)

    DERIVED_DIR.mkdir(parents=True, exist_ok=True)
    daily.to_csv(DERIVED_DIR / "tsageri_daily_clean.csv", index=False)
    monthly.to_csv(DERIVED_DIR / "tsageri_monthly_clean.csv", index=False)
