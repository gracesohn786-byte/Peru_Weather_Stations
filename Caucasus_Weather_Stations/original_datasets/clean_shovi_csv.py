...
SHOVI_CSV = BASE_DIR / "Shovi 1508m Lat42d42_ Long43d41_.csv"


def load_shovi_csv(path: Path) -> pd.DataFrame:
...
def aggregate_shovi_monthly(df_daily: pd.DataFrame) -> pd.DataFrame:
    df = df_daily.copy()
...
def main() -> None:
    if not SHOVI_CSV.exists():
        raise FileNotFoundError(f"Shovi CSV not found at {SHOVI_CSV}")

    raw = load_shovi_csv(SHOVI_CSV)
    ...
    monthly = aggregate_shovi_monthly(daily)

    DERIVED_DIR.mkdir(parents=True, exist_ok=True)
    daily.to_csv(DERIVED_DIR / "shovi_daily_clean.csv", index=False)
    monthly.to_csv(DERIVED_DIR / "shovi_monthly_clean.csv", index=False)
